...
static struct vfsmount *shm_mnt;    /* shmem 挂载点 */
...
#ifdef CONFIG_TMPFS /* 基于内存的虚拟文件系统 */
...
static void shmem_init_inodecache(void) /* inode 缓存 */
{
...
int shmem_init_fs_context(struct fs_context *fc)    /*  初始化*/
{
...
int __init shmem_init(void) /* 共享内存 */
{
	...
	shmem_init_inodecache();/* inode 缓存 */

	error = register_filesystem(&shmem_fs_type);    /* 注册一个文件系统 */
	if (error) {
		pr_err("Could not register tmpfs\n");
		goto out2;
	}

	shm_mnt = kern_mount(&shmem_fs_type);   /* 挂载文件系统 */
	if (IS_ERR(shm_mnt)) {
