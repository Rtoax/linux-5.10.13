...
atomic_long_t __cacheline_aligned_in_smp vm_node_stat[NR_VM_NODE_STAT_ITEMS] ;
...
{   /*  */
	...
	v = __this_cpu_inc_return(*p);  /* 得到 percpu变量的本CPU指针 */

	if (unlikely(v > NUMA_STATS_THRESHOLD)) {
		zone_numa_state_add(v, zone, item); /* 进行统计 */
		__this_cpu_write(*p, 0);    /* 写回 */
	}
}
...
int __read_mostly sysctl_stat_interval  = HZ;
