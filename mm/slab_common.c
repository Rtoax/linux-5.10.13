...
enum slab_state slab_state;/* slab 分配器 的状态 */
LIST_HEAD(slab_caches);
DEFINE_MUTEX(slab_mutex);
struct kmem_cache *kmem_cache;

#ifdef CONFIG_HARDENED_USERCOPY
bool __ro_after_init usercopy_fallback  =
...
bool slab_is_available(void)/*slab可用  */
{
	return slab_state >= UP;
}

#ifndef CONFIG_SLOB
/* Create a cache during boot when no slab services are available yet 创建启动缓存 */
void __init create_boot_cache(struct kmem_cache *s, const char *name,
		unsigned int size, slab_flags_t flags,
		unsigned int useroffset, unsigned int usersize)
{
	int err;
	unsigned int align = ARCH_KMALLOC_MINALIGN; /* 对齐 */

	s->name = name; /* 名称 */
	s->size = s->object_size = size;    /* 目标大小 */

	/*
	 * For power of two sizes, guarantee natural alignment for kmalloc
	 * caches, regardless of SL*B debugging options.
	 */
	if (is_power_of_2(size))    /* 补全 */
		align = max(align, size);
	s->align = calculate_alignment(flags, align, size); /* 对齐 */

	s->useroffset = useroffset;
	s->usersize = usersize;

	err = __kmem_cache_create(s, flags);    /* 创建缓存 */
...
static inline unsigned int size_index_elem(unsigned int bytes)  /* 8->0, 9->1, */
{
	return (bytes - 1) / 8;
}
