...
atomic_long_t __read_mostly _totalhigh_pages ;
...
} ____cacheline_aligned_in_smp page_address_htable[1<<PA_HASH_ORDER/* 7 */];
...
void __init page_address_init(void)/* start_kernel() */
{
	...
	for (i = 0; i < ARRAY_SIZE(page_address_htable)/* 1<<7=128 */; i++) {
		INIT_LIST_HEAD(&page_address_htable[i].lh);/* 初始化链表 */
		spin_lock_init(&page_address_htable[i].lock);   /* 初始化 锁 */
	}
}
