...
struct page *vmalloc_to_page(const void *vmalloc_addr)  /*  */
{
	...
	pgd_t *pgd = pgd_offset_k(addr);    /* 一级页表 */
	p4d_t *p4d;
	pud_t *pud;
	pmd_t *pmd;
	pte_t *ptep, pte;
	...
	if (pgd_none(*pgd))
		return NULL;
	p4d = p4d_offset(pgd, addr);
	if (p4d_none(*p4d)) /* p4d 页表 */
		return NULL;
	pud = pud_offset(p4d, addr);    /* up 页表 */
	...
	if (pud_none(*pud) || pud_bad(*pud))
		return NULL;
	pmd = pmd_offset(pud, addr);    /* middle 页 */
...
static struct vm_struct __initdata *vmlist ;/*  */
...
{
	static size_t __initdata vm_init_off ;
	unsigned long addr;
...
static void vmap_init_free_space(void)  /*  初始化 vmalloc 区域内存*/
{
...
{//
	 ...
	 */ /* vmap area */
	vmap_area_cachep = KMEM_CACHE(vmap_area, SLAB_PANIC/* 分配失败 panic */);

	for_each_possible_cpu(i) {  /* 遍历 CPU */
		struct vmap_block_queue *vbq;
		struct vfree_deferred *p;

		vbq = &per_cpu(vmap_block_queue, i);
		spin_lock_init(&vbq->lock); /* 初始化 自旋锁 */
	...
	for (tmp = vmlist; tmp; tmp = tmp->next) {
		...
		va->vm = tmp;
		insert_vmap_area(va, &vmap_area_root, &vmap_area_list); /* 插入红黑树 */
	}

	/*
	 * Now we can initialize a free vmap space.
	 */
	vmap_init_free_space(); /* 初始化 vmalloc 区域的内存 */
