...
static struct kmem_cache_node __initdata init_kmem_cache_node[NUM_INIT_LISTS];  /* 最大的 NUMA 节点 */
...
static bool __initdata slab_max_order_set ;  /*  */
...
static int __read_mostly use_alien_caches  = 1;  /* 使用 ailen cache */
 ...
 * before smp_init().
 */
void __init kmem_cache_init(void)/* slab 初始化 */
{
	int i;
	...
	if (!IS_ENABLED(CONFIG_NUMA) || num_possible_nodes() == 1)
		use_alien_caches = 0;   /* 远端 NODE 是否存在 */
	 ...
	 */
	if (!slab_max_order_set && totalram_pages()/* ram */ > (32 << 20) >> PAGE_SHIFT)
	...
	list_add(&kmem_cache->list, &slab_caches/* 机器上的所有 slab */);
	slab_state = PARTIAL;   /* SLUB */
	...
	/* 5) Replace the bootstrap kmem_cache_node */
	{
		...
		for_each_online_node(nid) {
			init_list(kmem_cache, &init_kmem_cache_node[CACHE_CACHE/* 0 */ + nid], nid);
