...
int sock_fd, sock_arp, flags = XDP_FLAGS_UPDATE_IF_NOEXIST;
...
static int recv_msg(struct sockaddr_nl sock_addr, int sock_fd)
{
	struct nlmsghdr *nh;
	int len, nll = 0;
	char *buf_ptr;
	...
	buf_ptr = buf;
	while (1) {
		len = recv(sock_fd, buf_ptr, sizeof(buf) - nll, 0);
		if (len < 0)
			return len;
...
static int monitor_route(void)
{
	...
	if (bind(sock_fd, (struct sockaddr *)&lr, sizeof(lr)) < 0) {
		printf("bind to netlink: %s\n", strerror(errno));
		ret = -1;
		goto cleanup;
	}
	fds_route.fd = sock_fd;
