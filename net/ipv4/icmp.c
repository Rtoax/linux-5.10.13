...
int __read_mostly sysctl_icmp_msgs_per_sec  = 1000;
int __read_mostly sysctl_icmp_msgs_burst  = 50;
...
static bool icmp_echo(struct sk_buff *skb)
{
	...
	_net = dev_net(skb_dst(skb)->dev);
	if (!_net->ipv4.sysctl_icmp_echo_ignore_all) {
		struct icmp_bxm icmp_param;
