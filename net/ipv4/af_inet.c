...
static int __init ipv4_offload_init(void)   /* offload: 将原本在协议栈中进行的IP分片，TCP分段、重组、chechsum校验 */
{
	/*
	 * Add offloads
	 */
...
fs_initcall(ipv4_offload_init); /*  IPv4 卸载 */

static struct packet_type __read_mostly ip_packet_type  = {
	.type = cpu_to_be16(ETH_P_IP),
	.func = ip_rcv,
...
fs_initcall(inet_init); /* INET 初始化 */
