...
__u32 __read_mostly sysctl_wmem_max  = SK_WMEM_MAX;
EXPORT_SYMBOL(sysctl_wmem_max);
__u32 __read_mostly sysctl_rmem_max  = SK_RMEM_MAX;
EXPORT_SYMBOL(sysctl_rmem_max);
__u32 __read_mostly sysctl_wmem_default  = SK_WMEM_MAX;
__u32 __read_mostly sysctl_rmem_default  = SK_RMEM_MAX;

/* Maximal space eaten by iovec or ancillary data plus some space */
int __read_mostly sysctl_optmem_max  = sizeof(unsigned long)*(2*UIO_MAXIOV+512);
EXPORT_SYMBOL(sysctl_optmem_max);

int __read_mostly sysctl_tstamp_allow_data  = 1;
			    ...
			    bool old_timeval)
{
	...
	if (tv.tv_sec < 0) {
		static int __read_mostly warned ;
...
int proto_register(struct proto *prot, int alloc_slab)  /* 注册协议(应用层和传输层接口) */
{
	int ret = -ENOBUFS;
	...
	list_add(&prot->node, &proto_list); /* 挂入链表 */
	mutex_unlock(&proto_list_mutex);
	return ret;
