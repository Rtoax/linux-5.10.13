...
#ifdef CONFIG_XPS
struct static_key __read_mostly xps_needed ;
EXPORT_SYMBOL(xps_needed);
struct static_key __read_mostly xps_rxqs_needed ;
...
int __read_mostly dev_rx_weight  = 64;
int __read_mostly dev_tx_weight  = 64;
/* Maximum number of GRO_NORMAL skbs to batch up for list-RX */
int __read_mostly gro_normal_batch  = 8;
...
struct rps_sock_flow_table __rcu __read_mostly *rps_sock_flow_table ;
EXPORT_SYMBOL(rps_sock_flow_table);
u32 __read_mostly rps_cpu_mask ;
EXPORT_SYMBOL(rps_cpu_mask);

struct static_key_false __read_mostly rps_needed ;
EXPORT_SYMBOL(rps_needed);
struct static_key_false __read_mostly rfs_needed ;
...
/* Called from hardirq (IPI) context *//* RPS-接收包控制 */
...
int __read_mostly netdev_flow_limit_table_len  = (1 << 12);
#endif
...
int __read_mostly (*br_fdb_test_addr_hook)(struct net_device *dev,
			     unsigned char *addr) ;
...
static __latent_entropy void net_rx_action(struct softirq_action *h)
{
	...
	for (;;) {
		...
		budget -= napi_poll(n, &repoll);    /* NAPI */
...
static int __net_init netdev_init(struct net *net)
{
	...
	net->dev_name_head = netdev_create_hash();  /* 创建哈希表 */
	...
	RAW_INIT_NOTIFIER_HEAD(&net->netdev_chain); /* 通知链 */
 ...
 */
static int __init net_dev_init(void)    /* 网络设备初始化 */
{
	int i, rc = -ENOMEM;
	...
	if (register_pernet_subsys(&netdev_net_ops))    /* 网络子系统 */
		goto out;
...
subsys_initcall(net_dev_init);  /* 网络设备初始化 */
