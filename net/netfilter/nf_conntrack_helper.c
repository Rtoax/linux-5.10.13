...
struct hlist_head __read_mostly*nf_ct_helper_hash ;
EXPORT_SYMBOL_GPL(nf_ct_helper_hash);
unsigned int __read_mostly nf_ct_helper_hsize ;
EXPORT_SYMBOL_GPL(nf_ct_helper_hsize);
static unsigned int __read_mostly nf_ct_helper_count ;

static bool __read_mostly nf_ct_auto_assign_helper  = false;
module_param_named(nf_conntrack_helper, nf_ct_auto_assign_helper, bool, 0644);
MODULE_PARM_DESC(nf_conntrack_helper,
		 "Enable automatic conntrack helper assignment (default 0)");

static DEFINE_MUTEX(nf_ct_nat_helpers_mutex);
static struct list_head __read_mostly nf_ct_nat_helpers ;
