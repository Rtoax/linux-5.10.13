...
static unsigned int __read_mostly sip_timeout  = SIP_TIMEOUT;
module_param(sip_timeout, uint, 0600);
MODULE_PARM_DESC(sip_timeout, "timeout for the master SIP session");

static int __read_mostly sip_direct_signalling  = 1;
module_param(sip_direct_signalling, int, 0600);
MODULE_PARM_DESC(sip_direct_signalling, "expect incoming calls from registrar "
					"only (default 1)");

static int __read_mostly sip_direct_media  = 1;
module_param(sip_direct_media, int, 0600);
MODULE_PARM_DESC(sip_direct_media, "Expect Media streams between signalling "
				   "endpoints only (default 1)");

static int __read_mostly sip_external_media  = 0;
...
static struct nf_conntrack_helper __read_mostly sip[MAX_PORTS * 4] ;
