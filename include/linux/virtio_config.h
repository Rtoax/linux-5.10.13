 ...
 */
typedef char * str_t;   /* +++ */
typedef struct cpumask * pcpumask_t; /* +++ */
typedef void vq_callback_t(struct virtqueue *);

struct virtio_config_ops {  /* 配置 virtio 设备的操作 */
	...
	int (*find_vqs)(struct virtio_device *, unsigned nvqs, struct virtqueue *vqs[], vq_callback_t *callbacks[],
        			const char * const names[], const bool *ctx, struct irq_affinity *desc);
	void (*del_vqs)(struct virtio_device *);
	u64 (*get_features)(struct virtio_device *vdev);
	int (*finalize_features)(struct virtio_device *vdev);
	const str_t (*bus_name)(struct virtio_device *vdev);
	int (*set_vq_affinity)(struct virtqueue *vq, const struct cpumask *cpu_mask);
	const pcpumask_t (*get_vq_affinity)(struct virtio_device *vdev, int index);
