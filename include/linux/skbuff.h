...
#define PKT_TYPE_OFFSET()	offsetof(struct sk_buff, __pkt_type_offset)
	 ...
	 *  可能的值在 include/linux/if_packet.h 中列出。对于以太网设备，此参数由函数
	 *  eth_type_trans 初始化。
	 *
	 *  主要的一些值：
	 *
	 *  1. PACKET_HOST: 接收到的帧的目的地址就是当前接收接口。也就是说，数据包已到
	 *     达其目的地
	 *  2. PACKET_MULTICAST: 接收到的帧的目标地址是当前接收接口已注册过的的多播地
	 *     址之一
	 */
	__u8			pkt_type:3;
	__u8			ignore_df:1;
...
#define PKT_VLAN_PRESENT_OFFSET()	offsetof(struct sk_buff, __pkt_vlan_present_offset)
	 ...
	 *  由于每个协议都有其自己的处理传入数据包的处理函数，因此驱动程序使用此字段来通知
	 *  其上一层使用什么处理函数。
	 *  每个驱动程序都调用 netif_rx 来调用上层网络层的处理函数，因此必须在调用该函数之
	 *  前初始化协议字段。
	 *
	 */
	__be16			protocol;
	 ...
	 *  每一层能够在 head 和 data 之间的区域填充协议头，或者在 tail 和 end 之间的
	 *  区域填充新的数据。
	 *
	 *                      +------------>+----------+
	 *                      |             | headroom |
	...
	/**
	 *  这个字段表示 buffer 的总大小，包括了 sk_buff 自己的占用。在执行 alloc_skb
	 *  函数时该字段被初始化。
	 *  `SKB_TRUESIZE()`
	 */
...
extern const struct skb_checksum_ops __read_mostly *crc32c_csum_stub ;
