 ...
 * 指令，编码还允许在需要时使用进一步的指令扩展集合。大端计算机上单个 64 位指令的
 * 指令编码定义为从最高有效位 （MSB） 到最低有效位 （LSB） 的位序列，分别为
 *
 * op：8、dst_reg：4、src_reg：4、off：16、imm：32
 *
 * 关闭和 IMM 是签名类型。编码是内核头文件的一部分，并在 linux/bpf.h 头文件中定
...
/**
 *  Key of an a BPF_MAP_TYPE_LPM_TRIE entry - 最长前缀匹配
 */
...
enum bpf_cmd {
	...
	/**
	 * BPF 映射和程序充当内核资源，只能通过文件描述符访问，由内核中的匿名索引节点支持。
	 * 优点，但也有一些缺点随之而来：
	 *
	 * 1. 用户空间应用程序可以利用大多数与文件描述符相关的 API，Unix 域套接字的文
	 *   件描述符传递透明地工作等，但同时，文件描述符仅限于进程的生存期，这使得映射共享
	 *   等选项执行起来相当麻烦。
	 * 2. 因此，它为某些用例带来了许多复杂性，例如 iproute2，其中 tc 或 XDP 设置
	 *   程序并将其加载到内核中并最终终止自身。这样，用户空间端也无法访问映射，否则它
	 *   可能很有用，例如，当映射在数据路径的入口和出口位置之间共享时。
	 * 3. 此外，第三方应用程序可能希望在 BPF 程序运行时监视或更新映射内容。
	 *
	 * 为了克服这个限制，已经实现了一个最小的内核空间BPF文件系统，其中BPF映射和程序
	 * 可以固定到，这个过程称为对象固定。因此，BPF 系统调用已使用两个新命令进行了扩展，
	 * 这两个命令可以固定 （BPF_OBJ_PIN） 或检索 （BPF_OBJ_GET） 以前固定的对象。
	 *
	 * 例如，tc 等工具利用此基础架构在入口和出口上共享地图。BPF 相关的文件系统不是单例，
	 * 它确实支持多个挂载实例、硬链接和软链接等。
	 */
	/**
...
enum bpf_map_type {
	 ...
	 *  数组映射
	 *  1. 不能删除
	 *  2. 数组映射上 使用 的 bpf_map_update_elem() 不是原子性的，
	 *     如果将计数器保存在数组中，可以使用内核的内置函数 __sync_fetch_and_add() 执行
	 */
	BPF_MAP_TYPE_ARRAY,
	/**
	 *  程序数组映射 - 是添加到内核的第一个专用映射，保存对BPF程序的映射(BPF程序的文件描述符)
	 *
	...
	/**
	 *  LPM Tire 映射 - 使用 最长前缀匹配(LPM)算法来查找映射元素
	 *
	 *  此算法用于路由器和其他设备的流量转发表上，用来匹配IP地址到特定的路由表项上
	 *  该映射要求键大小为 8的倍，范围(8-2048),
	 *
	 *  如果想实现自己的 键，内核提供了 `bpf_lpm_trie_key`数据结构
	 */
	BPF_MAP_TYPE_LPM_TRIE,
	/**
	 *  映射数组 和 映射哈希 - 用来保存其他映射的引用
	 *
	 *  仅支持间接级别的引用，不能保存映射的映射的映射(不支持多级)，确保不会存储无限连接的映射
	 */
	 ...
	 *  两种 保存内核中打开套接字引用的专用映射
	 *  这种类型映射可与帮助函数 `bpf_redirect_map()` 一起使用
	 *  在当前 XDP   程序到其他套接字的缓冲区之间转发套接字。
	 *
	 *  - bpf_redirect_map()
	 *  - bpf_sk_redirect_map()
	...
	/**
	 *  BPF_MAP_TYPE_CGROUP_STORAGE 和 BPF_MAP_TYPE_PERCPU_CGROUP_STORAGE
	 *  cgroup 存储 - 将 BPF 程序附加到 cgroup 上
	 *
...
enum bpf_prog_type {
	 ...
	 *
	 *  参见:   BPF_MAP_TYPE_CGROUP_STORAGE 和 BPF_MAP_TYPE_PERCPU_CGROUP_STORAGE
	 */
...
union bpf_attr {
	 ...
	 *  BPF_MAP_CREATE
	 *
	 * 映射是驻留在内核空间中的高效键/值存储。可以从 BPF 程序访问它们，以便在多
	 * 个 BPF 程序调用之间保持状态。它们也可以通过用户空间的文件描述符访问，并且
	 * 可以与其他 BPF 程序或用户空间应用程序任意共享。
	 *
	 * 相互共享映射的 BPF 程序不需要具有相同的程序类型，例如，跟踪程序可以与网络
	 * 程序共享映射。单个 BPF 程序目前可以直接访问多达 64 个不同的映射。
	 *
	 * 有一些具有每 CPU 和非每 CPU 风格的通用映射可以读取/写入任意数据，但也有
	 * 一些非通用映射与帮助程序函数一起使用。
	 *
	 * 目前可用的通用地图有
	 * 	BPF_MAP_TYPE_HASH、
	 ...
	 * 	BPF_MAP_TYPE_PERCPU_ARRAY、
	 * 	BPF_MAP_TYPE_LRU_HASH、
	 * 	BPF_MAP_TYPE_LRU_PERCPU_HASH和
	 * 	BPF_MAP_TYPE_LPM_TRIE。
	 * 它们都使用相同的一组通用 BPF 帮助程序函数来执行查找、更新或删除操作，同
	 * 时实现具有不同语义和性能特征的不同后端。
	 *
	 * 当前内核中的非泛型映射是
	 * 	BPF_MAP_TYPE_PROG_ARRAY、
	 ...
	 * 	BPF_MAP_TYPE_ARRAY_OF_MAPS
	 * 	BPF_MAP_TYPE_HASH_OF_MAPS。
	 * 例如，BPF_MAP_TYPE_PROG_ARRAY是一个数组映射，它包含其他 BPF 程序，
	 * BPF_MAP_TYPE_ARRAY_OF_MAPS和BPF_MAP_TYPE_HASH_OF_MAPS都包含指向
	 * 其他映射的指针，以便整个 BPF 映射可以在运行时以原子方式替换。这些类型的
	 * 映射解决了一个特定问题，该问题不适合仅通过 BPF 帮助程序函数实现，因为需
	 * 要在 BPF 程序调用中保持其他（非数据）状态。
	 *
	 * BPF 映射和程序充当内核资源，只能通过文件描述符访问，由内核中的匿名索引节点支持。
	 * 优点，但也有一些缺点随之而来：
	 *
