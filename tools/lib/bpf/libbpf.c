			       ...
			       struct bpf_core_spec *spec)
{
	...
	/**
	 * BPF_ENUMVAL_EXISTS, BPF_ENUMVAL_VALUE
	 */
 ...
 *    configurations.
 *
 * 1. 对于给定的本地类型，找到相应的候选目标类型。
 *    候选类型是具有相同“sample”名称的类型，忽略最后一个三重下划线 （___） 之后的所有内容。
 *    例如，“样本”、“sample___flavor_one”、“sample___flavor_another_one”都是彼此的
 *    候选者。带有三重下划线的名称被称为“风格”，除其他外，它可用于指定/支持同一内核结构的不兼
 *    容变体，这些变体在不同的内核版本和/或构建配置之间可能有所不同。
 *
 ...
 *
 *    注意：结构“flavors”可以由 bpftool 的 BTF 到 C 转换器生成，当内核的重复数据删除的
 *    BTF 仍然包含多个具有相同名称的不同类型时。在这种情况下，从第二个名称冲突开始附加
 *    ___2、___3 等。但是在 BPF 程序中“locally”定义启动风格也很有用，可以从不同内核
 *    版本/配置之间的不兼容更改中提取相同的数据。
 *    例如，要处理内核版本之间的字段重命名，可以使用具有相同公用名的两种结构名称，并使用条件
 *    重定位来提取该字段，具体取决于目标内核版本。
 *
 ...
 *    is not an error.
 *
 * 3. 支持并预期可能有多个与规范匹配的 spec。只要所有规范在所有候选项中解析为同一组偏移，就
 *    不会有错误。如果存在任何歧义，CO-RE 重新定位将失败。这对于适应 BTF 重复数据删除的不
 *    完全是必要的，如果某些直接或间接引用（通过指针）类型解析为不同对象文件中的不同实际类型，
 *    则可能会导致同一 BTF 类型的轻微重复。如果发生这种情况，重复数据删除的 BTF 最终将得
 *    到两个（或多个）结构相同的类型，它们仅在通过指针引用的类型上有所不同。这在大多数情况下
 *    应该是可以的，并且不是错误。
 *
