...
static char __initdata dmi_ids_string[128] ;
...
{
    //使用 `dmi_walk_early` 和 `dmi_alloc` 函数收集内存设备的有关信息
 ...
 *
 * `_SM_` 字符串一定在 `000F0000h` 和 `0x000FFFFF` 地址之间
 * 获得 `SMBIOS` 的版本和 `_DMI_` 的属性例如 `_DMI_` 的结构表长度、结构表的地址等
 */
...
/**
 *  `dmi_scan_machine` 函数通过 `dmi_early_remap` 函数
...
static void __init dmi_scan_machine(void)
{
    ...
    //首先使用 EFI(Unified_Extensible_Firmware_Interface)
	if (efi_enabled(EFI_CONFIG_TABLES)) {
		 ...
		 * `_SM_`(System Management) 字符串一定在 `000F0000h` 和 `0x000FFFFF` 地址之间
		 */
		memset(buf, 0, 16);
		for (q = p; q < p + 0x10000; q += 16) {
            //用 `memcpy_fromio` 函数向 `buf` 里面拷贝16个字节
			memcpy_fromio(buf + 16, q, 16);
			if (!dmi_present(buf)) {
				dmi_available = 1;
...
{
    //遍历 [System Management BIOS](http://en.wikipedia.org/wiki/System_Management_BIOS) 结构，并从中提取信息
	dmi_scan_machine();
	if (!dmi_available)
		return;
