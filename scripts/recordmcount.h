				 ...
				 Elf32_Word const *symtab_shndx)
{
	...
	ldebug("symtab_shndx = %d(SHN_XINDEX)\n", w(symtab_shndx[index]));
			...
			unsigned int const symsec_sh_link)
{
	...
	ldebug("make section __mcount_loc.\n");
	...
	ldebug("make section .rela__mcount_loc.\n");
			       ...
			       unsigned const reltype)
{
	...
	for (t = nrel; t; --t) {
		...
		if (mcountsym && mcountsym == Elf_r_sym(relp) &&
			...
			ldebug("addend = %08lx, r_offset = %6d\n", addend, mrelp->r_offset);
 ...
 * GNU 'as' for the start of each section.
 *
 * 在给定部分中找到一个符号，用作重新定位对 mcount 的调用的偏移量表的基础。
 * 局部或全局符号就足够了，但要避免使用弱符号，因为它可能会被覆盖；
 * 值的更改将使对 mcount 的调用的偏移量的重定位无效。
 * 通常，找到的符号将是 GNU 为每个部分的开头生成的未命名的本地符号“as”。
 *
 * For example:
	...
	for (relhdr = shdr0, k = nhdr; k; --k, ++relhdr) {
		...
		if (txtname && is_mcounted_section_name(txtname)) {
			...
			result = find_secsym_ndx(w(relhdr->sh_info), txtname,
						...
						ehdr);
			if (result) {
				lerror("failed to find_secsym_ndx()\n");
				goto out;
			}

			rel_entsize = _w(relhdr->sh_entsize);

			ldebug("rel_entsize = %d\n", rel_entsize);
