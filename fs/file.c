...
unsigned int __read_mostly sysctl_nr_open  = 1024*1024;//cat /proc/sys/fs/nr_open
...
{
	__set_bit(fd, fdt->open_fds);/* 设置 bitmaps 标志位 */
	fd /= BITS_PER_LONG;
	if (!~fdt->open_fds[fd])/* open_fs[fd] = fffffffffffffff 时，if 为真 */
		__set_bit(fd, fdt->full_fds_bits);/* 给 full 置位 */
}
