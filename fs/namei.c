...
int __read_mostly sysctl_protected_symlinks  = 0;
int __read_mostly sysctl_protected_hardlinks  = 0;
int __read_mostly sysctl_protected_fifos ;
int __read_mostly sysctl_protected_regular ;
 ...
 *
 * 执行（文件）名解析，也就是说就是开始处理一个给定的路径；
 * 从 `dcache` 更新当前的目录入口或询问底层文件系统
 */
...
path_openat(struct nameidata *nd, const struct open_flags *op, unsigned flags)
{
	 ...
	 */
		...
		while (!(error = link_path_walk(s, nd)) &&
				(s = open_last_lookups(nd, rtoax_file, op)) != NULL);
