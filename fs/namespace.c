...
static unsigned int __read_mostly mp_hash_mask ;
static unsigned int __read_mostly mp_hash_shift ;
...
/* /sys/fs */
struct kobject *fs_kobj; /* /sys/fs 文件系统 */
...
struct vfsmount *vfs_create_mount(struct fs_context *fc)    /*  */
{
	...
	mnt->mnt.mnt_sb		= fc->root->d_sb;   /* super_block */
...
struct vfsmount *vfs_kern_mount(struct file_system_type *type,
				int flags, const char *name/* 例 tmpfs */,
				void *data)
{
...
struct vfsmount *kern_mount(struct file_system_type *type)  /* 挂载文件系统 */
{
	struct vfsmount *mnt;
	mnt = vfs_kern_mount(type, SB_KERNMOUNT, type->name/* 例 tmpfs */, NULL);
	if (!IS_ERR(mnt)) {
		/*
