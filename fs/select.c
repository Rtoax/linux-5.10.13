...
static struct poll_table_entry *poll_get_entry(struct poll_wqueues *p)
{
	...
	if (!table || POLL_TABLE_FULL(table)) { /* 为空或者已满 */
			      ...
			      enum poll_time_type pt_type, int ret)
{
	...
	ktime_get_ts64(&rts);   /* 计算select返回时候的 time */
	...
	case PT_TIMEVAL:
		{
			...
			if (!copy_to_user(p, &rtv, sizeof(rtv)))    /* 将时间拷贝至用户态 */
				return ret;
		}
		break;
	case PT_OLD_TIMEVAL:
			   ...
			   fd_set __user *exp, struct timespec64 *end_time)
{
	...
	rcu_read_lock();
	fdt = files_fdtable(current->files);    /* 当前进程打开的文件描述符 */
	max_fds = fdt->max_fds; /* 最大 FD */
	rcu_read_unlock();
	if (n > max_fds)
		n = max_fds;    /* fd检查，提高性能 */
	...
	if (size > sizeof(stack_fds) / 6) {
		...
		alloc_size = 6 * size;
		bits = kvmalloc(alloc_size, GFP_KERNEL);    /* 栈上的 FDs 大小不够，申请 */
		if (!bits)
			goto out_nofds;
	}
		       ...
		       fd_set __user *exp, struct __kernel_old_timeval __user *tvp)
{
	...
	if (tvp) {  /* 超时选项 */
         ...
         *  计算超时时间
         */
		if (poll_select_set_timeout(to, /* 计算到时时间 */
		...
		int, timeout_msecs)
{
	...
	if (ret == -ERESTARTNOHAND) {
		struct restart_block *_restart_block;

		_restart_block = &current->restart_block;
		_restart_block->fn = do_restart_poll;
		_restart_block->poll.ufds = ufds;
		_restart_block->poll.nfds = nfds;

		if (timeout_msecs >= 0) {
			_restart_block->poll.tv_sec = end_time.tv_sec;
			_restart_block->poll.tv_nsec = end_time.tv_nsec;
			_restart_block->poll.has_timeout = 1;
		} else
			_restart_block->poll.has_timeout = 0;
