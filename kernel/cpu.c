...
enum cpuhp_smt_control __read_mostly cpu_smt_control  = CPU_SMT_ENABLED;
...
static struct smp_hotplug_thread cpuhp_threads = {
	.store			= &cpuhp_state.thread,/* struct task_struct **store; */
	.create			= &cpuhp_create,
...
void bringup_nonboot_cpus(unsigned int setup_max_cpus)
{
	...
	for_each_present_cpu(cpu) {
		if (num_online_cpus() >= setup_max_cpus)/* 在线的 CPU 大于 最大 CPU 数，直接退出 */
			break;
		if (!cpu_online(cpu))/* CPU 不在线 */
