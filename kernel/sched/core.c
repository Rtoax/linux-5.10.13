...
void do_set_cpus_allowed(struct task_struct *p, const struct cpumask *new_mask)
{
	...
	queued = task_on_rq_queued(p);  /* 任务是否在 运行队列上 */
...
{
	const struct sched_class *rtoax_class;
	struct task_struct *p;
...
{
	struct sched_param lp = { lp.sched_priority = 0 };
	struct task_struct *p;
	int retval;
...
bool __read_mostly sched_smp_initialized ;
...
void __init sched_init(void)    /* 调度初始化 */
{
	...
	init_schedstats();  /*  initialization of scheduler statistics */
 ...
 *
 * 举例：
 *  若进程 A 和进程 B 的 nice 都为 0，那么权重都是 1024(NICE_0_LOAD), 他们获得 50%
 *  的 CPU 时间，计算公式为:
 *
 *              1024/(1024+1024) = 50%
 *
