 ...
 *
 * update_curr CFS 核心函数
 *  用来更新当前进程的运行时间信息(更新进程的虚拟时间):
 *
 * @cfs_rq: 当前进程的就绪队列
 *
 * 步骤:
 *
 * 1. delta_exec = now - curr->exec_start;  计算出当前CFS运行队列的进程
 *    距离上次更新虚拟时间的差值.
...
static void update_curr(struct cfs_rq *cfs_rq)
{
	...
	/**
	 *  获取当前 就绪队列 保存的 clock_task 值
	 */
	u64 now = rq_clock_task(rq_of(cfs_rq));

	/**
	 *  delta_exec 计算 该进程从上次调用 update_curr() 函数到现在的时间差
	 */
	u64 delta_exec;
...
enqueue_entity(struct cfs_rq *cfs_rq, struct sched_entity *se, int flags)
{
	 ...
	 * update_curr().
	 *
	 * 之前在 task_fork_fair() 减去的 min_vruntime ，在这里加上
	 */
	if (renorm && curr)
...
} ____cacheline_aligned nohz ;/* 减小调度时钟中断 */
...
enum group_type {
	 ...
	 * tasks.
	 *
	 * 调度组正在运行的 进程数大于 group_capacity_factor
	 */
	group_overloaded
};
 ...
 *
 * 遍历最繁忙的就绪队列中所有的进程，找出适合被迁移的进程，然后让这些进程退出就绪队列
 * 返回值为 已经迁移的多少个进程
 */
...
/**
 *  初始化结构体 sched domain load balance stats - 传入值必须为 有效空间
 */
...
static struct sched_group *find_busiest_group(struct lb_env *env)
{
	...
	/**
	 *  如果没找到 最防盲的调度组或者最繁忙的调度组中没有正在运行的进程，
	 *  那么跳过该调度域
	 */
	...
	if (local->group_type == group_overloaded) {
		 ...
		 * busiest group don't try to pull any tasks.
		 *
		 * 如果本地调度组的平均负载比刚才计算出来的最繁忙调度组的平均负载还要大
		 * 那就 不需要做负载均衡
		 */
		if (local->avg_load >= busiest->avg_load)
			goto out_balanced;
		 ...
		 * domain average load.
		 *
		 * 如果本地调度的的平均负载比整个调度域的平均负载还要大，那就不需要做负载均衡。
		 */
	...
	if (busiest->group_type != group_overloaded) {
	    ...
	    /**
	     *  如果当前 CPU 不处于空闲状态，那么比较本地调度组的平均量化负载和最繁忙调度组
	     *  的平均量化负载
	     */
...
static int should_we_balance(struct lb_env *env)
{
	...
	/**
	 *  若调度组中有空闲的 CPU， 则优先使用空闲CPU
	 *  若调度组中午空闲的 CPU， 则使用调度组中第一个CPU
	 */
...
static void task_fork_fair(struct task_struct *p)
{
	...
	/**
	 *  虚拟时间为什么要减去 min_vruntime 呢? 不会造成 vruntime 变得很小 导致新进程恶意占用调度器吗?
	 **
	 *  新进程还没有添加到调度器中，加入调度器时会增加 min_vruntime 值。
	 *
	 *  换个角度思考：
	 *      新进程 在 place_entity 函数中得到了惩罚，惩罚的虚拟时间由 sched_vslice 计算，
	 *      在某种程度上也是为了防止新进程恶意占用 CPU；
	 *
	 *  在 `enqueue_entity()` 中会加上
	 */
	se->vruntime -= cfs_rq->min_vruntime;
	rq_unlock(rq, &rf);
}
