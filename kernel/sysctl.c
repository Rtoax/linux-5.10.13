...
static struct ctl_table kern_table[] = {    /* /proc/sys/kernel/ */
	...
	},
	{
		 ...
		 *
		 *  判断一个进程是否可以利用高速缓存的热度。
		 *  如果进程的运行时间(now - p->se.exec_start) 小于他，那么内核认为
		 *  他的数据还在高速缓存里.
	...
	},
	{
		 ...
		 *
		 *  设置在 SMP 负载均衡机制里每次最多可以从目标 CPU 迁移多少个 进程到 源
		 *  CPU 里。
...
static struct ctl_table fs_table[] = {  /* /proc/sys/fs/xxx */
	{
		.procname	= "inode-nr",
		.data		= &inodes_stat,
...
static struct ctl_table debug_table[] = {   /* /proc/sys/fs/ */
...
int __init sysctl_init(void)
{
	...
	hdr = register_sysctl_table(sysctl_base_table); /* /proc/sys/ */
