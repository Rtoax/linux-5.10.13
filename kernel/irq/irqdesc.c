...
static void __init init_irq_default_affinity(void)  /* IRQ默认的亲和性 */
{
...
int __init early_irq_init(void) /* 初始化外部中断 */
{
	int i, initcnt, node = first_online_node;/* 第一个NUMA节点 */
	struct irq_desc *desc;

	init_irq_default_affinity();    /* 全部标记为 11111... */
