 ...
 */
static struct task_struct __initdata *rb_threads[NR_CPUS] ;
...
static char __initdata rb_string[]  =
	"abcdefghijklmnopqrstuvwxyz1234567890!@#$%^&*()?+\\"
	"?+|:';\",.<>/?abcdefghijklmnopqrstuvwxyz1234567890"
	"!@#$%^&*()?+\\?+|:';\",.<>/?abcdefghijklmnopqrstuv";

static bool __initdata rb_test_started ;
...
static __init int rb_write_something(struct rb_test_data *data, bool nested)    /* ringbuffer 测试 */
{
	struct ring_buffer_event *event;
	struct rb_item *item;
	bool started;
...
static __init int test_ringbuffer(void)
{
	...
	for_each_online_cpu(cpu) {
		...
		rb_data[cpu].cpu = cpu;
		rb_data[cpu].cnt = cpu;
		rb_threads[cpu] = kthread_create(rb_test, &rb_data[cpu],    /* 创建线程 */
						 "rbtester/%d", cpu);
