...
static atomic_t __read_mostly nr_freq_events ;
static atomic_t __read_mostly nr_switch_events ;
static atomic_t __read_mostly nr_ksymbol_events ;
static atomic_t __read_mostly nr_bpf_events ;
static atomic_t __read_mostly nr_cgroup_events ;
static atomic_t __read_mostly nr_text_poke_events ;
 ...
 */
int __read_mostly sysctl_perf_event_sample_rate 	= DEFAULT_MAX_SAMPLE_RATE;

static int __read_mostly max_samples_per_tick 	= DIV_ROUND_UP(DEFAULT_MAX_SAMPLE_RATE, HZ);
static int __read_mostly perf_sample_period_ns 	= DEFAULT_SAMPLE_PERIOD_NS;

static int __read_mostly perf_sample_allowed_ns  =
...
int __read_mostly sysctl_perf_cpu_time_max_percent  = DEFAULT_CPU_TIME_MAX_PERCENT;
				...
				struct task_struct *task)
{
	...
	/* (perf_hw_context = 0, perf_sw_context = 1) */
	for_each_task_context_nr(ctxn) {
...
/**
 *  perf_event_open 中 perf file 的操作 ops
 */
static const struct file_operations perf_fops = {
...
static struct idr pmu_idr;  /* ID 2 Pointer */
...
/**
 * perf_event_open(2) - open a performance event, associate it to a task/cpu
 *
 ...
 *                  使用 group_fd 参数来指定 perf_event 的 group_leader：
 *                  group_fd >= 0 指定对于的 perf_event 为当前 group_leader，
 *                  group_fd == -1 创建新的 group_leader
 *
 * 在同时指定的情况下task维度优先于cpu维度，所以pid、cpu组合起来有以下几种情况：
 * -----------------------------------------------------------------------------
 * 组合1：pid >= 0, cpu >= 0。perf_event绑定到task维度的context。task在得到cpu调度
 *       运行的时候，context上挂载的本task相关的perf_event也开始运行。
 *       但是如果event指定的cpu不等于当前运行的cpu，event不会得到执行，这样就符合了这个
 *       组合的含义；
 * 组合2：pid >= 0, cpu == -1。perf_event绑定到task维度的context。只要task得到调度，
 *       该perf_event就会得到执行；
 * 组合3：pid == -1, cpu >= 0。perf_event绑定到cpu维度的context。只要该cpu运行，该
 *       perf_event就会得到执行。目前只有在cpu online的情况下才能绑定perf_event，
 *       cpu hotplug支持可能会有问题；
		...
		pid_t, pid, int, cpu, int, group_fd, unsigned long, flags)
{
	...
	/**
	 *  其他利用 系统调用 perf_event_open 打开的 fd
	 */
	if (group_fd != -1) {
