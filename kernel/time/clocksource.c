...
static void clocksource_select(void)
{
    //pass `false` that is meant that we will go through all entries of the `clocksource_list`
 ...
 * @freq:	clocksource frequency (cycles per second) divided by scale, 标识一秒钟中断发生次数
 *
...
void __clocksource_update_freq_scale(struct clocksource *cs, u32 scale, u32 freq)
{
	 ...
	 */
	if (freq) {
        //calculate `mult` and `shift` for the given clock source
		/*
