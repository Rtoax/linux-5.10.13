...
static LIST_HEAD(kthread_create_list);  /* 新建的进程将通过此链表被 kthreadd 创建 */
...
static inline void set_kthread_struct(void *kthread)/* 内核线程 */
{
	/*
						    ...
						    va_list args)
{
	...
	list_add_tail(&create->list, &kthread_create_list); /* 加入到此链表 */
					   ...
					   ...)
{
	...
	task = __kthread_create_on_node(threadfn, data, node, namefmt, args);/* 创建内核线程 */
	va_end(args);
...
EXPORT_SYMBOL(kthread_bind);/* 设置CPU掩码 */
...
/**
 * kthread_unpark - unpark(唤醒) a thread created by kthread_create().
...
int kthreadd(void *unused)  /* kthreadd 线程 2号进程， 管理调度其它的内核线程 */
{
