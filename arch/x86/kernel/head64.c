...
unsigned int __ro_after_init __pgtable_l5_enabled ;
unsigned int __ro_after_init pgdir_shift  = 39;
EXPORT_SYMBOL(pgdir_shift);
unsigned int __ro_after_init ptrs_per_p4d  = 1;
...
unsigned long __ro_after_init vmemmap_base  = __VMEMMAP_BASE_L4;
...
//__init 只在初始化阶段使用，并且它所使用的内存将会被释放
static void __init copy_bootdata(char *real_mode_data)
{
    ...
    //将 `real_mode_data` 拷贝进 `boot_params`
	memcpy(&boot_params, real_mode_data, sizeof(boot_params));

    //如果系统引导工具（bootloader）没能正确初始化 `boot_params` 中的某些成员的话，
    //那么在接下来调用的 `sanitize_boot_params` 函数中将会对这些成员进行清零
	sanitize_boot_params(&boot_params);

    //`get_cmd_line_ptr` 函数来得到命令行的地址
...
asmlinkage __visible void __init x86_64_start_kernel(char * real_mode_data)
{
	 ...
	 * 内核通过调用 `load_ucode_bsp` 函数来加载处理器微代码（microcode）
	 * 微代码(Microcode):用简单的硬件操作来模拟当时无法用技术直接实现的复杂指令。
	 * 指的是用软件技术来实现硬件设计的一种技术。
	 * 拥有智能功能的计算机部件所包含的程序称为微码。
	 */
	load_ucode_bsp();
...
static void set_bringup_idt_handler(gate_desc *idt, int n, void *handler)   /* 中断描述符表 */
{
...
static void startup_64_load_idt(unsigned long physbase) /* 加载中断描述附表 到 lidt 寄存器 */
{
	...
	native_load_idt(desc);  /* 加载到 lidt 寄存器中 */
}
...
void __head startup_64_setup_env(unsigned long physbase)
{
	...
	native_load_gdt(&startup_gdt_descr);    /* 加载 全局描述符表 到 lgdt 寄存器中 */
	...
	startup_64_load_idt(physbase);  /* 加载 中断描述符表 到 lidt 寄存器 */
}
