...
static unsigned long __ro_after_init cr4_pinned_bits ;
...
#define __aligned_ulong __aligned(sizeof(unsigned long))
...
static const struct cpu_dev *cpu_devs[X86_VENDOR_NUM] = {};/* 保存所有 x86  */
...
static const struct x86_cpu_id __initconst cpu_vuln_blacklist[]  = {
...
/*
 * Do minimum CPU detection early. 最小的 CPU 探测
...
//	c->x86_virt_bits = 32;
	...
	if (!have_cpuid_p())
		identify_cpu_without_cpuid(c);/* 只有 32 位会执行 */
