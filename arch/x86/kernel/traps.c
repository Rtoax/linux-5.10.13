...
//#include <asm/proto.h>
#endif
unsigned long __percpu system_vectors[];/* +++++ */
...
void exc_invalid_op(struct pt_regs *regs){/* 我加的 */}
...
void exc_coproc_segment_overrun(struct pt_regs *regs){/* 我加的 */}
DEFINE_IDTENTRY(exc_coproc_segment_overrun)
{
	do_error_trap(regs, 0, "coprocessor segment overrun",
		      X86_TRAP_OLD_MF, SIGFPE, 0, NULL);
}
void exc_invalid_tss(struct pt_regs *regs, int error_code){/* 我加的 */}
DEFINE_IDTENTRY_ERRORCODE(exc_invalid_tss)
{
	do_error_trap(regs, error_code, "invalid TSS", X86_TRAP_TS, SIGSEGV,
		      0, NULL);
}
void exc_segment_not_present(struct pt_regs *regs, int error_code){/* 我加的 */}
DEFINE_IDTENTRY_ERRORCODE(exc_segment_not_present)
{
	do_error_trap(regs, error_code, "segment not present", X86_TRAP_NP,
		      SIGBUS, 0, NULL);
}
void exc_stack_segment(struct pt_regs *regs, int error_code){/* 我加的 */}
DEFINE_IDTENTRY_ERRORCODE(exc_stack_segment)
{
	do_error_trap(regs, error_code, "stack segment", X86_TRAP_SS, SIGBUS,
		      0, NULL);
}
void exc_alignment_check(struct pt_regs *regs, int error_code){/* 我加的 */}
 ...
 */
    void exc_double_fault(struct pt_regs *regs){/* 我加的 */}
...
void exc_bounds(struct pt_regs *regs){/* 我加的 X86_TRAP_BR */}
...
DEFINE_IDTENTRY_ERRORCODE(exc_general_protection)
{
	 ...
	 *
	 * 如果不可抢占、kprobe在运行、kprobe->fault_handler执行成功
	 */
	if (!preemptible() &&
	    kprobe_running() &&
...
void exc_spurious_interrupt_bug(struct pt_regs *regs){/* 我加的 */}
...
void exc_device_not_available(struct pt_regs *regs){/* 我加的 */}
...
#ifdef CONFIG_X86_32
void iret_error(struct pt_regs *regs){/* 我加的 */}
