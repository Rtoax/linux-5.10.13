...
//		pcpu_chosen_fc = PCPU_FC_PAGE;
	...
	rc = -EINVAL;
    //检查 `per-cpu` 第一个块分配器
    //如果内核命令行中没有设置 `percpu_alloc` 参数，就会使用 `embed` 分配器
	if (pcpu_chosen_fc != PCPU_FC_PAGE) { /* pcpu_chosen_fc 在 percpu_alloc_setup() 中设置 */
...
//		atom_size = PAGE_SIZE;
#endif
        //`embed` 分配器,分配第一块空间,将第一个 per-cpu 块嵌入 bootmen
