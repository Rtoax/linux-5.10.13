		       ...
		       unsigned long address, u32 pkey, int si_code)
{
	...
	/* User mode accesses just cause a SIGSEGV ，如果是用户空间*/
...
/*
 * Handle a spurious(伪造的) fault caused by a stale(陈旧的) TLB entry.
 *
...
access_error(unsigned long error_code, struct vm_area_struct *vma)
{
	...
	/* read, present: */
	if (unlikely(error_code & X86_PF_PROT)) /* 可读，存在 */
		return 1;

	/* read, not present: */
	if (unlikely(!vma_is_accessible(vma)))  /* 可读，不存在 */
		return 1;
...
//	}
	...
	if (spurious_kernel_fault(hw_error_code, address))/* 一个伪造的 fault */
		return;
			...
			unsigned long address)
{
	...
	/* kprobes don't want to hook the spurious(模拟触发的) faults: */
	 ...
	 */
	if (user_mode(regs)) {  /* 如果是用户空间 */
		local_irq_enable(); /* 开启中断 */
		flags |= FAULT_FLAG_USER;
	} else {    /* 如果不是用户空间 */
		if (regs->flags & X86_EFLAGS_IF)/* 如果中断标志位置位 */
			local_irq_enable(); /* 开启中断 */
	}
...
DEFINE_IDTENTRY_RAW_ERRORCODE(exc_page_fault)
{
	...
	prefetchw(&current->mm->mmap_lock); /* 预取 读写信号量 */
