 ...
 * page table.
 */
static void __init pti_clone_user_shared(void)/* 页表隔离 */
{
	unsigned int cpu;
	...
	for_each_possible_cpu(cpu) {/* 遍历 CPU */
		...
		phys_addr_t pa = per_cpu_ptr_to_phys((void *)va);   /* 物理地址 */
		pte_t *target_pte;
		...
		*target_pte = pfn_pte(pa >> PAGE_SHIFT/* 12 */, PAGE_KERNEL);
	}
}
 ...
 */
//static void __init pti_clone_user_shared(void)/* 32bit */
//{
