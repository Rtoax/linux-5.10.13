...
static void add_identity_map(unsigned long start, unsigned long end)
{
	...
	/* Align boundary to 2M. 它对齐内存到 2MB 边界并检查给定的起始地址和终止地址 */
...
void initialize_identity_maps(void *rmode)
{
	 ...
	 * 初始化 pgt_buf_offset
	 * 由于我们只是在初始化阶段，`initialize_identity_maps`函数设置`pgt_buf_offset`为0
	 */
	 ...
	 *
	 * `pgt_data.pgt_buf_size`会根据引导加载器所用的引导协议（64位或32位）被设置为`77824`或`69632`
	 */
