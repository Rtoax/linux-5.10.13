...
//所有不安全的内存区域将被收集在一个称为数组中mem_avoid：
//MEM_AVOID_MAX 代表不同类型的保留内存区域的 mem_avoid_index 枚举
...
//`slots_fetch_random`函数的主要目标是通过`kaslr_get_random_long`函数从`slot_areas`数组选择随机的内存范围
 ...
 * `process_efi_entries`函数的主要目标是在整个可用的内存找到所有的合适的内存区域来加载内核。
 */
static bool
					   ...
					   unsigned long image_size)
{
    ...
    //所有找到的内存区域会存储在 slot_areas 数组中。
    //内核解压器应该选择这个数组随机的索引，并且它会是内核解压的随机位置,
    //这个选择会被`slots_fetch_random`函数执行.
 ...
 * @output      - 它是`startup_32`对齐到 2MB 边界的地址 解压后的内核镜像的位置(物理地址)
 * @output_size - z_output_len = 83868872   解压后的内核镜像的长度
 * @virt_addr   - virt_addr = LOAD_PHYSICAL_ADDR 内核加载地址的虚拟地址(虚拟地址)
 * @
 *
