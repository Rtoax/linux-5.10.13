...
//0x80第一个硬盘驱动器在哪里，EDD_MBR_SIG_MAX宏的值为16。
//它将数据收集到edd_info结构的数组中。
//get_edd_info 通过调用0x13带有ahas的中断来检查EDD是否存在0x41，
//如果存在EDD，则 get_edd_info 再次调用该0x13中断，但是使用ahas 0x48 并si包含将存储EDD信息的缓冲区的地址。
