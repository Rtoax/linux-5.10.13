...
struct cpuinfo_x86 {/* x86 架构 CPU 信息 */
...
#endif
	__u8			x86_virt_bits;
	__u8			x86_phys_bits;  /* 代表系统可达到的最大内存地址时需要的比特数 */
	 ...
	 *
	 * 可能有 X86_FEATURE_XXX , 如 `X86_FEATURE_APIC`
	 */
	union {
...
static inline void load_cr3(pgd_t *pgdir)/* CR3 寄存器保存全局页表 */
{
	write_cr3(__sme_pa(pgdir));/* init_task 的 全局页表 写入 CR3 寄存器 */
}
...
extern asmlinkage void ignore_sysret(void);//仅返回 `-ENOSYS` 错误代码
...
static inline unsigned int cpuid_ecx(unsigned int op)   /* 获取寄存器值 */
{
	unsigned int eax, ebx, ecx, edx;
 ...
 * space during mmap's.
 */
#define __TASK_UNMAPPED_BASE(task_size)	(PAGE_ALIGN(task_size / 3)) // 用户空间的1/3处，再加上随机偏移
