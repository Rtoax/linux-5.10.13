 ...
 * 基数-0 段的基址为0
 * 限制-0xfffff   （段的大小）为- 0xfffff（1 MB）
 * 标志-0xc09b    1100 0000 1001 1011
 *      1-（G）粒度位
 *      1-（D） 如果为0 16位段；1 = 32位段
 *              如果为1，则在64位模式下执行
 *      0-（L）
 *      0-（AVL）可供系统软件使用
 *      0000-描述符中的4位长度19:16位
 *      1-（P）段 内存中存在
 *      00-（DPL）-特权级别，0是最高特权
 *      1-（S）代码或数据段，而不是系统段
 *      101-段类型执行/读取/
...
#define GDT_ENTRY_BOOT_CS	2   //代码段
#define GDT_ENTRY_BOOT_DS	3   //数据段
#define GDT_ENTRY_BOOT_TSS	4   //任务状态段
...
#define GDT_ENTRY_INVALID_SEG	0   //[Global Descriptor Table]的无效段
...
/* Needs two entries */
#define GDT_ENTRY_TSS			8   /* `Task State Segments` */
/* Needs two entries */
 ...
 *
 * 每一项为 `9` 个字节，其中：
 * 2个字节的备用指令用于向栈中压入默认错误码（如果异常本身没有提供错误码的话），
 * 2个字节的指令用于向栈中压入向量号，剩余5个字节用于跳转到异常处理程序。
 */
 ...
 *  每个描述符都为8个字节
 *  把IDT中的每一项叫做 `门(gate)`
 *  CPU使用一个特殊的 `GDTR` 寄存器来存放全局描述符表的地址，
 *  中断描述符表也有一个类似的寄存器 `IDTR`
 */
