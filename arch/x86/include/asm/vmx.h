 ...
 */
#define CPU_BASED_CR3_LOAD_EXITING		VMCS_CONTROL_BIT(CR3_LOAD_EXITING)//VMX_FEATURE_CR3_LOAD_EXITING
...
enum vmcs_field {
	...
	/**
	 *  如果 KVM 发现虚拟中断芯片有中断请求，则向 VMCS 中 VM-entry control 部分的
	 *  VM-entry interrupt information field 字符安写入中断信息，在切入 Guest
	 *  模式的一刻，CPU 将检查这个字段，就如同检查 CPU 引脚，如果有中断，则进入中断处理
	 *  执行过程
	 *
	 *  见 `vmx_inject_irq()`
	 *
